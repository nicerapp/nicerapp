nicerapp.photoAlbum = na.pa = {
	...
	printAlbum_handleFileLoad : function (event) {
		...
		file = event.item.id.replace(na.m.globals.urls.app+'siteData/nicerapp/com/ui/photoAlbum/albums/','').replace('/lowres',''),
		p = file.lastIndexOf('/'),
		...
		div.children('a').attr('href', na.m.globals.urls.app + 'photo/' + event.item.id.replace(na.m.globals.urls.app+'siteData/nicerapp/com/ui/photoAlbum/albums/','').replace('/lowres','').replace(/\//g,'_') + '--' + i);
