...
/**
Dispatched when the view is changed, e.g. from `list` to `thumbs` or vice verna.
...
$.widget("ui.plupload", {
	...
	_initUploader: function() {
		...
		uploader.bind('UploadFile', function(up, file) {
            //window.top.jQuery.cookie ('uploadFileRelativePath', file.relativePath);
			self._handleFileStatus(file);
            /*for (var k=0; k<files.length; k++) {
                var relPath = data.files[k].webkitRelativePath;
                if (typeof relPath=='string' && relPath!=='') {
                    //jQuery('#filePath')[0].value = relPath;
                    window.top.na.tree.currentNode_createPath ({
                        target:jQuery('#sasm___siteManager table', window.top.document)[0]
	...
	*/
	...
	_addFiles: function(files) {
		...
		$.each(files, function(i, file) {
            ...
            if (typeof relPath=='string' && relPath!=='') {
                //jQuery('#filePath')[0].value = relPath;
                var rp = relPath.substr(1).replace(/[^\/]*$/,'').replace(/\/$/,'');
                if (rp!=='') {
                    window.top.na.blog.currentNode_createPath (null, rp);
                }
			}
