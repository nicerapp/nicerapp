...
class Server
{
    ...
    public function useDb($options) : Database
    {
        ...
        if (isset($options['_security'])) {
            $security = $options['_security'];
        } else {
            $security = false;
        }
        //var_dump ($security); die();
    ...
    public function addAdmin ($name, $password) {
        $endpoint = "/_node/couchdb@127.0.0.1/_config/admins/".$name;
        $verb = "PUT";
        
        $response = $this->client->request($verb, $endpoint, ['json'=>$password]);
